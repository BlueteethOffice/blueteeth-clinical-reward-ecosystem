...
export default function DashboardLayout({ 
  ...
  React.useEffect(() => {
    const updateProfile = () => {
      ...
      if (localProfile) {
        ...
        setDisplayName(userData?.name || user.displayName || 'Doctor Name');
        setDisplayPhoto(userData?.photoURL || user.photoURL || '');
        if (userData?.role === 'admin') setDisplaySpec('Master Admin');
      }
    };
  ...
  return (
    <div className="fixed inset-0 h-[100svh] min-h-[100svh] bg-slate-50 flex overflow-hidden relative">
      ...
      <div className="hidden lg:flex lg:flex-shrink-0 lg:w-72 lg:flex-col lg:fixed lg:inset-y-0 lg:z-50 m-4">
        <div className="flex flex-col flex-1 min-h-0 bg-blue-900 border border-white/10 rounded-xl p-5 shadow-2xl">
          <div className="flex items-center flex-shrink-0 mb-10 px-2">
            <div className="h-10 w-10 bg-white rounded-xl flex items-center justify-center shadow-lg shadow-white/5">
              <span className="text-blue-900 font-black text-xl italic">B</span>
            </div>
          ...
          <nav className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
            {navigation.map((item) => (
              <Link key={item.name} href={item.href}>
                <motion.div 
                  whileHover={{ x: 5 }}
                  className={`group flex items-center px-6 py-2.5 text-sm font-bold rounded-xl transition-all duration-300 ${
                    isActive(item.href) 
          ...
          <div className="mt-auto mb-6 p-4 rounded-xl bg-white/5 border border-white/10 group overflow-hidden transition-all hover:bg-white/[0.08]">
             <div className="flex items-center gap-3">
                <div className="h-10 w-10 bg-white rounded-lg flex items-center justify-center shrink-0 shadow-lg shadow-white/5 overflow-hidden transition-all group-hover:scale-105">
                  {displayPhoto ? (
                ...
                <div className="flex flex-col min-w-0">
                  <p className="text-[11px] font-black text-white truncate leading-none">
                    {displayName.startsWith('Dr.') ? displayName : (isAdminRoute ? displayName : `Dr. ${displayName}`)}
                  </p>
                  <p className="text-[9px] font-bold text-blue-300/80 uppercase tracking-tight mt-1 truncate">
                    {isAdminRoute ? 'System Administrator' : displaySpec}
                  </p>
                </div>
             </div>
          </div>
          ...
          <div className="pt-6 border-t border-white/5 flex justify-start">
            ...
            >
                <div className="relative flex items-center gap-3">
                  ...
                  <span className="tracking-widest uppercase text-[9px]">Security Exit</span>
                </div>
            </Button>
          </div>
        </div>
      ...
      {/* Main content area */}
      <div className="flex flex-col flex-1 lg:ml-80 min-w-0 h-full overflow-y-auto overflow-x-hidden bg-slate-50 overscroll-contain pb-10 sm:pb-0 no-scrollbar">
  ...
  return (
    <>
      <input 
        ...
        className="relative z-0 pl-10 pr-24 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-400 transition-all w-80 h-10 shadow-sm"
        value={searchValue}
        onChange={(e) => setSearchValue(e.target.value)}
      />
      <Search 
        className="absolute left-3.5 h-4 w-4 text-slate-400 transition-colors z-10" 
      />
    </>
  );
}
    ...
    <>
      <div className="relative">
        <button 
         ...
         className={`relative p-2.5 rounded-xl transition-all ${
           showNotifications ? 'bg-blue-600 text-white' : 'text-slate-500 hover:bg-slate-50'
         }`}
        >
            ...
            <>
              ...
              >
                <div className="p-4 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
                   <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em]">Notification Hub</h3>
                ...
                <div className="max-h-96 overflow-y-auto">
                  {notifications.length > 0 ? notifications.map((notif: any) => (
                    ...
                    <div className="p-8 text-center text-slate-400 text-xs font-bold">No new alerts.</div>
                  )}
                </div>
                <Link href={isAdminRoute ? "/admin/notifications" : "/doctor/notifications"}>
                  <div onClick={() => setShowNotifications(false)} className="w-full block text-center p-4 text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-blue-600 transition-colors border-t border-slate-50 bg-slate-50/30">
                    View All
                  </div>
                </Link>
              </motion.div>
      ...
      {isAdminRoute ? (
        <div className="relative group flex items-center gap-3 cursor-pointer">
          ...
          <div className="h-9 w-9 bg-blue-50 rounded-full border border-blue-200 overflow-hidden shrink-0">
            {displayPhoto ? <img src={displayPhoto} className="h-full w-full object-cover" /> : <UserCircle className="h-6 w-6 text-blue-500/60 m-1.5" />}
          </div>
