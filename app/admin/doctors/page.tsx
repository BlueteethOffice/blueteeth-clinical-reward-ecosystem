...
function DoctorListContent() {
  ...
  const handleAdjustBalance = async () => {
    ...
    try {
      ...
      await addDoc(collection(db as any, 'cases'), {
        ...
        patientName: "OFFICIAL ADMIN CREDIT",
        treatment: "Authorized Wallet Adjustment",
        points: pointsChange,
        bonusPoints: 0,
        status: "Approved",
  ...
  return (
    <DashboardLayout isAdminRoute={true}>
      <Suspense fallback={<div className="space-y-6 animate-pulse p-8"><div className="h-20 bg-slate-100 rounded-xl"/><div className="h-64 bg-slate-100 rounded-xl"/></div>}>
        <div className="space-y-6 pb-4 md:pb-0" suppressHydrationWarning={true}>
        {/* Simple Compact Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4" suppressHydrationWarning={true}>
          <div className="space-y-1">
            <h1 className="text-2xl font-black text-blue-900 tracking-tight">Associate Partner List</h1>
            <p className="text-slate-500 font-medium text-xs">Manage your associate network and their points from here.</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
            <Button
              ...
              onClick={() => {
                ...
                link.setAttribute("download", "Blueteeth_Associates_List.csv");
                document.body.appendChild(link);
                link.click();
                toast.success('Associate list downloaded');
              }}
            ...
            >
              <UserPlus className="h-3.5 w-3.5" /> Add New Associate
            </Button>
          </div>
        </div>
            ...
            <table className="w-full text-left" suppressHydrationWarning={true}>
              <thead suppressHydrationWarning={true}>
                <tr className="bg-slate-50/50 border-b border-slate-100" suppressHydrationWarning={true}>
                  <th className="px-6 py-4 text-[9px] font-black text-blue-400 uppercase tracking-widest w-[35%]" suppressHydrationWarning={true}>Associate Details</th>
                  <th className="px-6 py-4 text-[9px] font-black text-blue-400 uppercase tracking-widest text-center" suppressHydrationWarning={true}>Contact Phone</th>
                  <th className="px-6 py-4 text-[9px] font-black text-blue-400 uppercase tracking-widest text-center" suppressHydrationWarning={true}>Total Cases</th>
                  <th className="px-6 py-4 text-[9px] font-black text-blue-400 uppercase tracking-widest text-center" suppressHydrationWarning={true}>Points Balance</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  [...Array(5)].map((_, i) => (
                    <tr key={i} className="animate-pulse"><td colSpan={5} className="h-16 bg-slate-50/10"></td></tr>
                  ))
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                       <span className="text-xs font-black text-slate-700">{doc.phone || doc.mobile || 'N/A'}</span>
                    </td>
                    <td className="px-6 py-4 text-center">
                       <div className="flex flex-col items-center">
                          <span className="text-xs font-black text-blue-600">0 Cases</span>
                          <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter">Lifetime Activity</span>
                       </div>
                    </td>
                    <td className="px-6 py-4 text-center">
          ...
          <div className="block lg:hidden divide-y divide-slate-100" suppressHydrationWarning={true}>
            {loading ? (
              ...
              <motion.div
                ...
                className="p-5 active:bg-slate-50 transition-colors flex items-center justify-between group"
              >
                <div className="flex items-center gap-4 min-w-0">
                   <div className="h-14 w-14 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-600 border-2 border-blue-100 shadow-sm overflow-hidden shrink-0 group-active:scale-95 transition-transform">
                      {doc.photoURL ? (
                        <img src={doc.photoURL} alt="p" className="h-full w-full object-cover" />
                      ) : (
                        <div className="h-full w-full flex items-center justify-center bg-blue-600 text-white text-sm font-black uppercase">
                          {doc.name?.charAt(0) || <Users size={20} />}
                        </div>
                      )}
                   </div>
                   <div className="min-w-0 flex-1">
                      <p className="font-black text-slate-900 uppercase tracking-tight truncate pr-2 text-sm">{doc.name || 'Practitioner'}</p>
                      <p className="text-[10px] font-bold text-slate-400 mt-0.5 truncate flex items-center gap-1">
                         <Mail size={10} className="shrink-0" /> {doc.email}
                      </p>
                      <div className="flex items-center gap-3 mt-3">
                         <div className="flex flex-col">
                            <span className="text-[11px] font-black text-emerald-600 leading-none">₹{Math.round((doc.totalPoints || 0) * exchangeRate).toLocaleString()}</span>
                            <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest mt-1">Earnings</span>
                         </div>
                         <div className="h-6 w-px bg-slate-100" />
                         <div className="flex flex-col">
                            <span className="text-[11px] font-black text-blue-600 leading-none">{Number(doc.totalPoints || 0).toFixed(1)}</span>
                            <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest mt-1">B-PTS</span>
                         </div>
                         {doc.phone && (
                           <>
                             <div className="h-6 w-px bg-slate-100" />
                ...
                <div className="h-10 w-10 flex items-center justify-center bg-slate-50 rounded-xl border border-slate-100 shrink-0">
          ...
          <>
            ...
            >
              ...
              <div className="p-6 border-b border-slate-50 flex items-center justify-between">
                <div>
                  <h2 className="text-sm font-black text-blue-900 uppercase tracking-widest">Associate Profile</h2>
                  ...
                  <div className="space-y-4">
                     ...
                     <div className="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
                        <p className="text-[9px] font-black text-indigo-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                           <Activity size={12} /> Associate Details
                        </p>
                        <div className="space-y-4">
                           <div className="flex items-center justify-between">
                      ...
                      {showAuditLogs && (
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="px-5 sm:px-8 py-4 sm:py-6 bg-slate-50 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative">
                              <div className="flex items-center gap-3 sm:gap-4">
                                ...
                                <div className="min-w-0">
                                  ...
                                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mt-1 truncate">Associate: <span className="text-blue-600 truncate">{selectedDoctor?.name}</span></p>
                                </div>
                              </div>
                            ...
                            {/* Professional Table View with Live Stats */}
                            <div className="flex-1 flex flex-col min-h-0">
                              {(() => {
                                const activeDoctor = doctors.find(d => d.id === selectedDoctor?.id) || selectedDoctor;
                                return (
                                  <div className="grid grid-cols-2 sm:grid-cols-4 border-b border-slate-50 shrink-0">
                                    <div className="p-3 sm:p-4 border-r border-b sm:border-b-0 border-slate-50 text-center">
                                      <p className="text-[7px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 leading-none">Wallet Points</p>
                                      <p className="text-xs sm:text-xl font-black text-slate-900 leading-none">{Number(activeDoctor?.totalPoints || 0).toFixed(1)} <span className="text-[8px] sm:text-[10px] text-slate-400 uppercase font-bold">PTS</span></p>
                                    </div>
                                    <div className="p-3 sm:p-4 border-b sm:border-b-0 border-slate-50 text-center">
                                      <p className="text-[7px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 leading-none">Total Wealth</p>
                                      <p className="text-xs sm:text-xl font-black text-slate-900 leading-none">₹{(activeDoctor?.walletBalance || 0).toLocaleString()}</p>
                                    </div>
                                    <div className="p-3 sm:p-4 border-r border-slate-50 text-center">
                                      ...
                                      <p className="text-xs sm:text-xl font-black text-slate-900 leading-none">{doctorCases.length} <span className="text-[8px] sm:text-[10px] text-slate-400 uppercase font-bold">NODES</span></p>
                                    </div>
                                    <div className="p-3 sm:p-4 text-center bg-blue-50/30">
                                      ...
                                      <p className="text-xs sm:text-xl font-black text-blue-600 leading-none">₹{exchangeRate} <span className="text-[8px] sm:text-[10px] text-blue-400 uppercase font-bold">/ PT</span></p>
                                    </div>
                                  </div>
                                );
                              })()}
                                    ...
                                    doctorCases
                                      ...
                                      .map((c, i) => (
                                        <div
                                          ...
                                          className="px-4 sm:px-8 py-4 sm:py-3 flex flex-col sm:grid sm:grid-cols-5 gap-3 sm:gap-4 items-start sm:items-center hover:bg-slate-50/80 transition-all cursor-pointer group border-b border-slate-50 relative"
                                        >
                                          {/* Patient & Treatment */}
                                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 min-w-0 w-full sm:w-auto">
                                            <p className="text-[11px] sm:text-[12px] font-black text-slate-900 uppercase group-hover:text-blue-600 transition-colors truncate">{c.patientName}</p>
                                            <div className="hidden sm:block h-4 w-px bg-slate-200" />
                                            <p className="text-[8px] sm:text-[10px] font-bold text-slate-400 uppercase truncate pr-1">{c.treatment}</p>
                                          </div>

                                          {/* Points & Cash Stack for Mobile */}
                                          <div className="flex items-center justify-between w-full sm:contents">
                                             <div className="sm:text-center">
                                               <p className="text-[11px] sm:text-[12px] font-black text-blue-600">
                                             ...
                                             <div className="sm:text-center">
                                               <p className="text-[11px] sm:text-[12px] font-black text-slate-900 truncate">₹{Math.round((Number(c.points) + Number(c.bonusPoints || 0)) * exchangeRate).toLocaleString()}</p>
                                             </div>
                                          </div>

                                          <div className="flex items-center justify-between w-full sm:contents mt-1 sm:mt-0">
                                             <div className="flex justify-center sm:w-full">
                                               <span className={`px-2 sm:px-3 py-1 rounded-lg text-[8px] font-black border uppercase tracking-widest ${
                                             ...
                                             <div className="flex justify-end items-center gap-2">
                                               ...
                                               <button
                                                 ...
                                                 className="h-8 w-8 sm:h-7 sm:w-7 rounded-lg border border-rose-50 text-rose-300 hover:text-white hover:bg-rose-500 hover:border-rose-500 transition-all flex items-center justify-center bg-white"
                                               >
                                                 <Trash2 size={12} />
                                               </button>
                                             </div>
                          ...
                          >
                            ...
                            <div className="p-4 space-y-2.5">
                              ...
                              <div 
                                onClick={() => {
                                  ...
                                  if (imgWindow) {
                                    imgWindow.document.write(`
                                      <html>
                                        <head><title>${selectedAuditCase.solutionUrl ? 'Clinician Solution' : 'Clinical Evidence'} - Blueteeth</title></head>
                                ...
                                className={`relative h-24 group overflow-hidden rounded-2xl border shadow-inner cursor-pointer transition-all ${selectedAuditCase.solutionUrl ? 'bg-emerald-50 border-emerald-200 hover:bg-emerald-100' : 'bg-slate-100 border-slate-200 hover:bg-blue-50'}`}
                              >
                                {selectedAuditCase.solutionUrl || selectedAuditCase.evidenceUrl ? (
                                  ((selectedAuditCase.solutionUrl || selectedAuditCase.evidenceUrl).startsWith('data:application/pdf') || (selectedAuditCase.solutionUrl || selectedAuditCase.evidenceUrl).toLowerCase().includes('.pdf')) ? (
                                    <div className="w-full h-full flex flex-col items-center justify-center">
                                      <FilePlus className={`h-8 w-8 mb-1 group-hover:scale-110 transition-transform ${selectedAuditCase.solutionUrl ? 'text-emerald-500' : 'text-rose-500'}`} />
                                      <p className={`text-[9px] font-black uppercase tracking-widest ${selectedAuditCase.solutionUrl ? 'text-emerald-600' : 'text-rose-600'}`}>
                                        {selectedAuditCase.solutionUrl ? 'View Solution' : 'Open Evidence'}
                                      </p>
                                    </div>
                                  ) : (
                                    <img
                                      src={selectedAuditCase.solutionUrl || selectedAuditCase.evidenceUrl}
                                  ...
                                  <div className="w-full h-full flex flex-col items-center justify-center p-8 text-center bg-slate-50">
                                    ...
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-relaxed">Official Record Sync</p>
                                  </div>
                                )}
                                <div className="absolute top-2 right-2 flex gap-1">
                                  {selectedAuditCase.solutionUrl && (
                                    <div className="bg-emerald-500 text-white px-1.5 py-0.5 rounded text-[6px] font-black uppercase shadow-lg shadow-emerald-500/20">
                                      Solved
                                    </div>
                                  )}
                                </div>
                              ...
                              <div className="space-y-2">
                                ...
                                {/* Dynamic Bonus Adjustment Tool - LOCKED UNTIL SOLVED */}
                                {(() => {
                                  const isWaitingForClinician = selectedAuditCase.status === 'Assigned' && !selectedAuditCase.solutionUrl;
                                  
                                  return (
                                    <div className={`p-3 rounded-xl border space-y-2 transition-all ${isWaitingForClinician ? 'bg-slate-50 border-slate-100 opacity-60' : 'bg-blue-50 border-blue-100'}`}>
                                      <p className="text-[8px] font-black text-blue-600 uppercase tracking-widest leading-none text-left flex items-center gap-2">
                                        {isWaitingForClinician ? <Lock size={10} className="text-slate-400" /> : <Activity size={10} />} 
                                        Clinical Bonus Protocol
                                      </p>
                                      
                                      {isWaitingForClinician ? (
                                        <div className="py-2 text-center">
                                          <p className="text-[7px] font-black text-slate-400 uppercase tracking-widest">Waiting for Clinician Solution</p>
                                          <p className="text-[6px] font-bold text-slate-300 uppercase mt-0.5">Controls are locked for integrity</p>
                                        </div>
                                      ) : (
                                        <div className="space-y-1.5">
                                          <input
                                            id="bonusInput" type="number" step="0.1" placeholder="+ Points"
                                            disabled={selectedAuditCase.status === 'Approved'}
                                            className="w-full px-3 py-1.5 rounded-lg bg-white border border-blue-200 text-xs font-black text-blue-900 outline-none focus:ring-2 focus:ring-blue-400 disabled:cursor-not-allowed"
                                          />
                                          <textarea
                                            id="reasonInput" placeholder="Audit Justification"
                                            disabled={selectedAuditCase.status === 'Approved'}
                                            className="w-full px-3 py-1.5 rounded-lg bg-white border border-blue-200 text-[9px] font-bold text-blue-900 outline-none focus:ring-2 focus:ring-blue-400 h-8 resize-none disabled:cursor-not-allowed"
                                          />
                                          <button
                                            onClick={() => {
                                            ...
                                            disabled={adding || selectedAuditCase.status === 'Approved'}
                                            className="w-full py-2 bg-blue-600 text-white rounded-lg text-[9px] font-black uppercase hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/20 disabled:opacity-50 disabled:bg-slate-400 disabled:shadow-none"
                                          >
                                            {adding ? 'Syncing...' : (selectedAuditCase.status === 'Approved' ? 'ALREADY AUTHORIZED' : 'Authorize Clinical Asset')}
                                          </button>
                                        </div>
                                      )}
                                    </div>
