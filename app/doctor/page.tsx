...
export default function AssociateDashboard() {
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // [PERFORMANCE] Persist to Cache (Resilient)
      try {
        localStorage.setItem(`clinical_stats_${user.uid}`, JSON.stringify(statsPayload));
        // Only cache basic metadata for first 10 items
        const fastCases = sortedCases.slice(0, 10).map(c => ({
          id: c.id,
        ...
        localStorage.setItem(`clinical_cases_${user.uid}`, JSON.stringify(fastCases));
      } catch (e) {
        console.warn("Storage Full. Clearing dashboard cache.");
    ...
    <DashboardLayout>
      ...
      >
        ...
        <div className="flex flex-col gap-6 lg:gap-8">
          ...
          <div className="flex flex-nowrap sm:flex-wrap items-center gap-1 sm:gap-3 bg-white/30 p-1 rounded-[4px] border border-slate-100/50 backdrop-blur-xl w-[calc(100%+8px)] -ml-1 sm:ml-0 sm:w-full">
            <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 rounded-[4px] bg-emerald-500/10 backdrop-blur-md border border-emerald-500/20 shadow-sm transition-all group shrink-0 sm:shrink">
              <div className={`h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full ${connectionStatus === 'Connected' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : connectionStatus === 'Disconnected' ? 'bg-red-500' : 'bg-amber-500 animate-pulse'}`} />
              <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-emerald-700/80 whitespace-nowrap">Status: {connectionStatus}</span>
            </div>
            <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 rounded-[4px] bg-white/40 backdrop-blur-md border border-slate-200/50 shadow-sm text-slate-600 transition-all shrink-0 sm:shrink">
              {connectionStatus === 'Connected' ? <Wifi size={10} className="text-emerald-500 sm:w-[14px] sm:h-[14px]" /> : <WifiOff size={10} className="text-red-400 sm:w-[14px] sm:h-[14px]" />}
              <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-slate-700/70 leading-none whitespace-nowrap">
                <span className="sm:hidden">Level: </span><span className="hidden sm:inline">System Level: </span>
                {connectionStatus === 'Connected' ? 'Live' : 'Offline'}
              </span>
            </div>
            <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 rounded-[4px] bg-indigo-500/10 backdrop-blur-md text-indigo-700/90 text-[8px] sm:text-[10px] font-black uppercase tracking-widest border border-indigo-500/20 shadow-sm leading-none shrink-0 sm:shrink">
              <CheckCircle2 size={10} className="text-indigo-600 sm:w-3 sm:h-3" /> <span className="sm:hidden">Secure Sync</span><span className="hidden sm:inline">Secure Login Verified</span>
            </div>
          </div>

          {/* Associate Welcome Header */}
          <div className="w-full">
            <div className="flex flex-col space-y-1.5 sm:space-y-3">
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="-mt-1 inline-flex items-center gap-1 sm:gap-2 self-start px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-[4px] bg-blue-50/50 backdrop-blur-md border border-blue-100 text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-blue-600 shadow-sm leading-none">
                <Activity className="h-2.5 w-2.5 sm:h-3 sm:w-3" /> <span className="hidden sm:inline">System Status: </span>{loading ? 'UPDATING...' : 'REAL-TIME SYNC'}
              </motion.div>
              <h1 className="text-2xl sm:text-4xl font-black text-slate-900 tracking-tight leading-tight max-w-full break-words">
                <span className="sm:hidden text-slate-800">Hi </span><span className="hidden sm:inline">Welcome, </span><span className="bg-gradient-to-r from-blue-600 via-indigo-600 to-cyan-500 bg-clip-text text-transparent">
                  {(userData?.role === 'admin') ? 
                    (userData?.name === 'Master Admin Niteen02' ? 'Master Admin' : (userData?.name || 'Admin')) : 
                    (userData?.name || user?.displayName ? 
                      (String(userData?.name || user?.displayName).replace(/^Dr\.\s*/i, '')) 
                      : 'Associate'
                    )
                  }
                </span>
              </h1>
              <p className="text-[9px] sm:text-[11px] font-bold text-slate-500 sm:text-slate-600 uppercase tracking-[0.15em] leading-none">
                <span className="hidden sm:inline">Associate Rewards Dashboard • </span><span className="sm:hidden">Rewards Ledger • </span>{new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }).toUpperCase()}
              </p>
            </div>
          </div>
          ...
          <div className="order-3 lg:order-3 w-full">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-8">
              <div className="flex flex-row items-center gap-4 sm:gap-6 py-4 lg:pt-6 border-t border-slate-100 lg:border-t-0 w-full lg:w-auto">
                <div className="flex items-center gap-2 sm:gap-3 group cursor-default min-w-0">
                  <div className="h-8 w-8 rounded-full bg-blue-600 flex items-center justify-center shadow-lg shadow-blue-200 border border-blue-400 shrink-0">
                    <User size={14} className="text-white" />
                  </div>
                  <div className="flex flex-col min-w-0">
                    <span className="hidden sm:block text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Associate Account</span>
                    <span className="text-[11px] sm:text-[12px] font-black text-slate-800 truncate max-w-[150px] sm:max-w-none">
                      <span className="sm:hidden">{user?.email?.split('@')[0] || 'Guest'}</span>
                      <span className="hidden sm:inline">{user?.email || 'Guest User'}</span>
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 sm:gap-3 group cursor-default shrink-0">
                  <div className="h-8 w-8 rounded-full bg-indigo-600 flex items-center justify-center shadow-lg shadow-indigo-200 border border-indigo-400 shrink-0">
                    <ShieldCheck size={14} className="text-white" />
                  </div>
                  <div className="flex flex-col">
                    <span className="hidden sm:block text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Identity Verified</span>
                    <span className="text-[11px] sm:text-[12px] font-black text-blue-600 uppercase tracking-tight">ID: {(user?.uid || '').slice(0, 8).toUpperCase()}</span>
                  </div>
                </div>
              </div>

              <Link href="/doctor/submit-case" className="block w-full lg:w-auto">
                <Button className="w-full h-14 sm:h-16 px-6 rounded-[4px] bg-blue-600 hover:bg-blue-700 text-white shadow-xl shadow-blue-500/20 flex items-center justify-between gap-4 transition-all duration-500 active:scale-95 group relative overflow-hidden">
                  ...
                  <div className="bg-white/20 p-2 rounded-[4px] group-hover:bg-white/30 transition-colors relative z-10 shrink-0"><Plus className="h-5 w-5 sm:h-6 sm:w-6" /></div>
                  <div className="text-center relative z-10 flex-1">
                    <span className="block font-black text-xs sm:text-base uppercase tracking-widest">New Case Submission</span>
                  </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-2 sm:gap-6 w-full">
          {stats.map((item, idx) => (
            <motion.div
              key={item.name}
              ...
              className={`bg-white p-3 sm:p-5 rounded-[4px] border border-slate-100 shadow-sm hover:shadow-xl transition-all group border-t-4 ${getCaseBorderClass(item.name)}`}
            >
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                <div className={`h-8 w-8 sm:h-12 sm:w-12 rounded-[4px] flex items-center justify-center shrink-0 ${item.bg} group-hover:scale-110 transition-transform shadow-inner`}><item.icon className={`h-4 w-4 sm:h-6 sm:w-6 ${item.color}`} /></div>
                <div className="flex flex-col min-w-0">
                  <p className="text-[7px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5 truncate">{item.name}</p>
                  <h3 className="text-sm sm:text-xl font-black text-slate-900 tracking-tighter truncate">{item.value}</h3>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        <div className="pt-2 sm:pt-6">
          <div className="flex items-center justify-between mb-4 sm:mb-8 w-full p-4 sm:p-4 bg-gradient-to-r from-blue-600 to-blue-900 rounded-[4px] shadow-xl relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full blur-3xl -mr-16 -mt-16" />
            <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0 relative z-10">
              <div className="h-8 w-8 sm:h-10 sm:w-10 bg-white/10 rounded-[4px] flex items-center justify-center border border-white/10 shadow-lg shrink-0 leading-none">
                <Activity className="h-3.5 w-3.5 sm:h-5 sm:w-5 text-white" />
              </div>
              <div>
                <h2 className="text-sm sm:text-xl font-black text-white tracking-tight truncate whitespace-nowrap leading-none">Recent Activity</h2>
                <p className="text-[7px] sm:text-[9px] font-black text-blue-100 uppercase tracking-widest mt-0.5 opacity-80 leading-none italic">Associate Ledger</p>
              </div>
            </div>
             <div className="flex items-center gap-2 sm:gap-4 ml-2 sm:ml-4 flex-shrink-0 relative z-10">
              <Link href="/doctor/cases">
                <button className="text-[7px] sm:text-[9px] font-black text-white uppercase tracking-widest bg-white/10 backdrop-blur-md border border-white/10 px-2.5 py-2 rounded-[4px] sm:rounded-md hover:bg-white/20 shadow-sm whitespace-nowrap">Archive</button>
              </Link>
              <div className="h-6 w-6 sm:h-8 sm:w-8 rounded-[4px] bg-white flex items-center justify-center text-blue-600 border border-white shadow-lg shrink-0">
                <Plus size={14} />
              </div>
            </div>
          </div>

          <div className="bg-white rounded-[4px] border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden w-full">
                ...
                paginatedCases.map((c) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <p className="text-[15px] font-black text-slate-900 leading-tight uppercase tracking-tight">{c.patientName}</p>
                        <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mt-1">REF: {(c.id || '').slice(0, 6).toUpperCase()}</p>
                      </div>
                      <span className={`inline-flex px-2.5 py-1 rounded-[4px] text-[8px] font-black uppercase tracking-widest border ${c.status === 'Approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                    ...
                    paginatedCases.map((c) => (
                      ...
                      >
                        ...
                        <td className="px-8 py-3.5 text-center">
                          <span className={`inline-flex items-center gap-2 rounded-[4px] px-3 py-1 text-[8.5px] font-black uppercase tracking-widest border ${c.status === 'Approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' :
            ...
            {totalPages > 1 && (
              <div className="px-4 sm:px-8 py-4 sm:py-5 border-t border-slate-50 bg-slate-50/30 flex items-center justify-center sm:justify-between gap-2">
                <p className="hidden sm:block text-[10px] font-black text-slate-700 uppercase tracking-widest whitespace-nowrap shrink-0">
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <Button
                    ...
                    className="h-9 w-9 sm:h-8 sm:w-8 p-0 rounded-[4px] border-slate-200 text-slate-600 disabled:opacity-30 shadow-sm"
                  >
                    <ChevronLeft size={16} className="sm:w-[14px] sm:h-[14px]" />
                  </Button>
                  {[...Array(totalPages)].map((_, i) => (
                    <button
                      key={i}
                      onClick={() => setCurrentPage(i + 1)}
                      className={`h-9 w-9 sm:h-8 sm:w-8 rounded-[4px] text-[11px] sm:text-[10px] font-black transition-all ${currentPage === i + 1
                  ...
                  <Button
                    ...
                    className="h-9 w-9 sm:h-8 sm:w-8 p-0 rounded-lg border-slate-200 text-slate-600 disabled:opacity-30 shadow-sm"
                  >
                    <ChevronRight size={16} className="sm:w-[14px] sm:h-[14px]" />
                  </Button>
                </div>
              </div>
            )}
      ...
      <AnimatePresence>
        {selectedCase && (
          <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 overscroll-contain">
            ...
            >
              <div
                className="pointer-events-auto w-full max-w-[500px] bg-white rounded-[4px] shadow-2xl overflow-hidden flex flex-col max-h-[92vh] border border-slate-100"
              ...
              >
                ...
                <div className={`px-6 sm:px-8 py-3.5 sm:py-5 shrink-0 relative overflow-hidden ${getCaseGradient(selectedCase.id)}`}>
                  ...
                  <div className="flex items-center justify-between relative z-10">
                    ...
                    <button
                      ...
                      className="h-8 w-8 sm:h-10 sm:w-10 rounded-[4px] bg-white/20 hover:bg-white/30 flex items-center justify-center transition-all active:scale-90"
                    >
                  ...
                  {selectedCase.evidenceUrl && (
                    <div className="pt-0.5 sm:pt-1">
                      ...
                      <div 
                        ...
                        className="relative group bg-slate-50 rounded-[4px] border-2 border-slate-100 overflow-hidden min-h-[50px] sm:min-h-[70px] flex items-center justify-center p-2 sm:p-4 cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-all active:scale-[0.99]"
                      >
                ...
                <div className="p-5 sm:p-8 pt-0 pb-6 sm:pb-8 shrink-0">
                  <button
                    onClick={() => setSelectedCase(null)}
                    className="w-full h-11 sm:h-14 rounded-[4px] bg-slate-900 hover:bg-slate-800 active:scale-[0.98] text-white text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] transition-all shadow-xl shadow-slate-200"
                  >
                    Go Back
                  </button>
                </div>
