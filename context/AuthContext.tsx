    ...
    try {
      ...
      if (lastUid) {
        ...
        if (snapshot) {
           ...
           // We load UI aesthetic data, including role if available
           setUserData({ name: parsed.name, photoURL: parsed.photoURL, role: parsed.role || 'associate' }); 
        }
      }
    } catch (e) {}
  }, []);
        ...
        userDocUnsubscribe = onSnapshot(userRef, (snapshot) => {
          if (snapshot.exists()) {
            ...
            localStorage.setItem(`clinical_identity_snapshot_${authUser.uid}`, JSON.stringify({
               role: data.role || (verifiedAdmin ? 'admin' : 'associate'),
               name: data.name,
